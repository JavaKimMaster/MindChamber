...
public class NaverSearchServiceImpl implements NaverSearchService {
    ...
    public NaverTrendResponse getTrend2(NaverTrendRequest requestDto, String naverClientID, String naverClientSecret) {
        ...
        try {
            ...
            // Embedded 복합키 구성으로 인해 responseBody를 넘긴다.
            if ( responseBody != null && naverTrendHandler.saveTrendData2(responseBody) ) {
                LOGGER.info("naverTrendHandler.saveTrendData2(results) 호출");
                return responseBody;
            } else {
                LOGGER.info("naverTrendHandler.saveTrendData2 실패");
                return null;
            }
